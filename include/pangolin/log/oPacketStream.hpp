...
class PANGOLIN_EXPORT oPacketStream
{
    ...
    public:
	oPacketStream():_stream(&_buffer),_open(false),_bytes_written(0){_stream.exceptions(std::ostream::badbit);};
	oPacketStream(const std::string& filename, size_t buffer_size  = 100*1024*1024):_buffer(pangolin::PathExpand(filename), buffer_size), _stream(&_buffer), _open(_stream.good()),_bytes_written(0)
	{
	...
	void writePacket(PacketStreamSourceId src, const unsigned char* source, size_t sourcelen) {writePacket(src, reinterpret_cast<const char*>(source), sourcelen);};
	void writePacket(PacketStreamSourceId src, const char* source, size_t sourcelen);
